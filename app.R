...
ui <- fluidPage(
   ...
   sidebarLayout(
      sidebarPanel(
        ...
        tags$a(href="https://drive.google.com/file/d/1QOwuIiJoDh5VuNbdkbcdYQ0IDITcwBOL/view?usp=sharing", "Link to example zip code file"),
        ...
        tags$a(href="mailto:johnh3@juno.com", "Email johnh3@juno.com to request password"),
        textInput(inputId="watermark", label="Remove Watermark Password", value = "***")
        #downloadButton("downloadPlot", "Download Plot") #never got download button figured out
      ),
  ...
  #     })
   ...
   output$mapPlot <- renderPlot({
     ...
     finalprintplot<-
      ...
      finalprintplot + theme(plot.margin = unit(c(1.5,.06,1,.125), "cm"),plot.background = element_rect(fill = input$var_backgroundcolor)) #adjust margins to get close to 8:10 final aspect ratio
